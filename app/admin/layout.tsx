...
export default function AdminLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex h-screen w-full bg-paper text-ink">
      {/* Sidebar */}
      <aside className="w-64 border-r border-line bg-sand flex flex-col">
        <div className="p-6">
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto bg-coffee">
...
function SidebarLink({ href, icon: Icon, children }: { href: string; icon: any; children: ReactNode }) {
  return (
    <Link
      href={href}
      className="flex items-center gap-3 px-4 py-3 rounded-xl text-ink/70 hover:text-ink hover:bg-ink/5 transition-colors"
    >
