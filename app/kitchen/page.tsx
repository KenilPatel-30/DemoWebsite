...
export default function KitchenPage() {
  ...
  if (loading) {
    return (
      <div className="flex w-full h-full items-center justify-center text-white/50 gap-4">
  ...
  return (
    <div className="flex gap-6 h-full w-full">
      {COLUMNS.map(col => {
        ...
        return (
          <div key={col.id} className="flex flex-col w-[350px] flex-shrink-0 bg-[#1a1a1a] rounded-2xl border border-white/5 overflow-hidden">
            <div className={`p-4 border-b-2 ${col.color} bg-[#121212] flex justify-between items-center`}>
              <h2 className="font-bold text-lg">{col.label}</h2>
              <span className="bg-white/10 px-3 py-1 rounded-full text-sm font-medium">{columnOrders.length}</span>
            </div>
            
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
              {columnOrders.length === 0 ? (
                <div className="text-center text-white/30 pt-10 text-sm">No orders in this queue</div>
              ) : (
                columnOrders.map(order => (
                  <div key={order.id} className="bg-[#222] rounded-xl p-4 shadow-lg border border-white/5 hover:border-white/20 transition-colors">
                    <div className="flex justify-between items-start mb-3 border-b border-white/5 pb-3">
                      ...
                      <div className="text-right">
                        <div className="font-medium text-sm text-white/80">{order.customerName}</div>
                        <div className="text-xs text-white/40 mb-1">{order.items.length} items</div>
                        {order.paymentMethod && (
                      ...
                      {order.items.map((item, idx) => (
                        <li key={idx} className="text-sm">
                          <div className="flex items-start gap-2">
                            <span className="font-bold bg-white/10 px-2 rounded">{item.quantity}x</span>
                    ...
                    <button 
                      ...
                      className={`w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 ${
                        ...
                        col.id === "Preparing" ? "bg-primary hover:bg-primary/80 text-black" :
