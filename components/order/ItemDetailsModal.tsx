...
function ModalContent({ item }: { item: OrderItem }) {
  const { setActiveItem, addToCart } = useOrder();
  
  // Local state for selections
  const [size, setSize] = useState(item.sizes?.[0]?.label || "");
  const [milk, setMilk] = useState(item.milkTypes?.[0] || "");
  const [sugar, setSugar] = useState(item.sugarLevels?.[0] || "");
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain min-h-0 pb-[100px]">
            ...
            <div className="px-6 py-6">
              <div className="flex justify-between items-start mb-2">
                <h2 className="text-[24px] font-medium text-ink leading-tight pr-4">{item.name}</h2>
                <span className="text-[18px] font-bold text-[#9A5015]">₹{item.price}</span>
              </div>
                ...
                <div className="pt-2">
                  ...
                  {showAllergens && (
                    <div className="bg-[#f2e6db]/50 p-4 rounded-2xl text-[14px] text-ink/80 mt-2">
                      {item.allergens && item.allergens.length > 1 ? (
...
export default function ItemDetailsModal() {
  ...
  return (
    <AnimatePresence>
      {activeItem && <ModalContent key="modal" item={activeItem} />}
    </AnimatePresence>
  );
}
